                ...
                if not fil:
                    ...
                    with open(fil.filename, 'r') as f:
                        for line in open("Routes.csv"):
                            ...
                            csv_row=[row for row in csv.reader(f.read().splitlines())]
                            for csv_row in csv_row:
                                y=Route(routeNumber=csv_row[0],routeName=csv_row[1],departureCity=csv_row[2],departureCode =csv_row[3],desinationCode =csv_row[4],routeType =csv_row[5],departureHour =csv_row[6],departureMin =csv_row[7],travelTimeHour =csv_row[8],travelTimeMin =csv_row[9])
                                print(y.id)
                                #driver_collection.insert(json.dumps(x))
                                Route.save(y)
...
def route_lookup():
    ...
    route_results= Assignment.objects(routeNumber=route_exists)
...
def route_exist_lookup():
    ...
    route_exists_results= Route.objects(departureCity=route_exist_dept, desinationCode=route_exist_dest )
